...
describe('ProductsService', () => {
    ...
    describe('getProducts', () => {
        it('should return an array of products', async () => {
            const products: any = [{ id: 1, name: 'Product 1' }, { id: 2, name: 'Product 2' }];
        ...
        it('should throw an error if products cannot be obtained', async () => {
            jest.spyOn(cacheService, 'get').mockResolvedValue(null);
            jest.spyOn(prismaService.product, 'findMany').mockRejectedValue(new Error('Database error'));

            await expect(productsService.getProducts()).rejects.toThrow(HttpException);
            expect(cacheService.get).toHaveBeenCalledWith('products');
            expect(prismaService.product.findMany).toHaveBeenCalled();
        });
    });
    ...
    describe('createProduct', () => {
        ...
        it('should throw an error if product cannot be created', async () => {
            ...
            jest.spyOn(prismaService.product, 'create').mockRejectedValue(new Error('Database error'));

            await expect(productsService.createProduct(createProductDto)).rejects.toThrow(HttpException);
            expect(prismaService.product.create).toHaveBeenCalledWith({
                data: {
                    name: createProductDto.name,
                    price: createProductDto.price,
    ...
    describe('getProductById', () => {
        ...
        it('should throw an error if product cannot be obtained', async () => {
            const id = 1;
            jest.spyOn(cacheService, 'get').mockResolvedValue(null);
            jest.spyOn(prismaService.product, 'findUnique').mockRejectedValue(new Error('Database error'));

            await expect(productsService.getProductById(id)).rejects.toThrow(HttpException);
            expect(cacheService.get).toHaveBeenCalledWith(`product:${id}`);
            expect(prismaService.product.findUnique).toHaveBeenCalledWith({
                where: {
                    id: id,
    ...
    describe('updateProduct', () => {
        ...
        it('should throw an error if product cannot be updated', async () => {
            ...
            jest.spyOn(prismaService.product, 'update').mockRejectedValue(new Error('Database error'));

            await expect(productsService.updateProduct(id, updateProductDto)).rejects.toThrow(HttpException);
            expect(prismaService.product.update).toHaveBeenCalledWith({
                where: {
                    id: id,
                },
